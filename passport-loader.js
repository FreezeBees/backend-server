...
// Signup passport
passport.use('signup', new LocalStrategy({ usernameField: 'StudentId', passwordField: 'password', passReqToCallback: true },
  async (req, uname, pass, done) => {
    const {
      name, email, password, phone, StudentId, RoleId, residence,
    } = req.body;
    console.log(req.body);
    UserModel.findOne({ where: { StudentId } }).then((user) => {
      ...
      const data = {
        name, email, password: hashpass, phone, StudentId, RoleId, residence,
      };
      console.log(data);
...
// Login passport
passport.use('login', new LocalStrategy({ usernameField: 'StudentId', passwordField: 'password' },
  async (StudentId, password, done) => {
    try {
      // Find the user associated with the email/username provided by the user
      const user = await UserModel.findOne({ where: { StudentId } });
      ...
      return done(null, user, { message: 'Logged in Successfully', name: user.name });
    } catch (error) {
